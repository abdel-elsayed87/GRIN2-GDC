...
def prep_gene_lsn_data(lsn_data, gene_data, mess_freq=10):
    ...
    print(f"Formatting gene position data for counting: {datetime.now()}")
    ...
    print(f"Formatting lesion position data for counting: {datetime.now()}")
    ...
    print(f"Combining formatted gene and lesion position data: {datetime.now()}")
    ...
    print(f"Verifying structure of combined gene and lesion data: {datetime.now()}")
    ...
    print(f"Verified correct construction and indexing of combined lesion and gene data: {datetime.now()}")
...
def find_gene_lsn_overlaps(gl_data):
    ...
    m = len(gene_lsn_data)
    print(f"Scanning through combined lesion and gene data to find gene-lesion overlaps: {datetime.now()}")
    ...
    print(f"Completed scan of combined gene-lesion data.: {datetime.now()}")
...
def prob_hits(hit_cnt, chr_size):
    ...
    for i in range(b):
        ...
        print(
            f"Computing p-values for {n_genes} gene(s) on chromosome {chrom} affected by {len(lsn_rows)} {lsn_type} (data block {i + 1} of {b}): {datetime.now()}")
    ...
    print(f"Computing q-values: {datetime.now()}")
    ...
    # to round the p and q values (save time and memory)
    round_digits = 3
